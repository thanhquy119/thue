...
test("invalidates browser search results created before the latest document routing change", () => {
  const source = readFileSync(new URL("../app/cache-version.tsx", import.meta.url), "utf8");
  assert.match(source, /2026-07-26-structured-source-fallback-v1/u);
