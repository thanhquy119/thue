...
test("invalidates browser search results when transfer reader improvements ship", () => {
  const source = readFileSync(new URL("../app/cache-version.tsx", import.meta.url), "utf8");
  assert.match(source, /2026-07-29-transfer-reader-v5/u);
  ...
  assert.doesNotMatch(source, /2026-07-29-device-aware-transfer-v4/u);
});
