...
test("invalidates browser search results when safe OCR transfer ships", () => {
  const source = readFileSync(new URL("../app/cache-version.tsx", import.meta.url), "utf8");
  assert.match(source, /2026-07-29-transfer-safe-ocr-v8/u);
  ...
  assert.doesNotMatch(source, /2026-07-29-transfer-full-pdf-v7/u);
});
