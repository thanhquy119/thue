...
const TimelineVisual = ({scene}: {scene: LegalVideoScene}) => {
  ...
  if (items.length === 1) {
    ...
    return <div style={{width: '100%', minHeight: 560, borderRadius: 52, backgroundColor: COLORS.sky, border: `2px solid ${COLORS.line}`, display: 'grid', gridTemplateColumns: '290px 1fr', alignItems: 'center', padding: '58px 54px', boxShadow: '0 24px 62px rgba(36,88,74,.10)'}}>
      <div style={{width: 240, height: 240, borderRadius: 72, display: 'grid', placeItems: 'center', backgroundColor: COLORS.card, border: `2px solid ${COLORS.line}`, scale: spring({frame: frame - VISUAL_ENTER_DELAY, fps, config: {damping: 23, stiffness: 95}})}}><ClockGlyph size={170} /></div>
...
const FlowVisual = ({scene}: {scene: LegalVideoScene}) => {
  ...
  if (items.length === 1) {
    const item = items[0];
    const appear = spring({frame: frame - VISUAL_ENTER_DELAY, fps, config: {damping: 24, stiffness: 96}});
  ...
  return <div style={{width:'100%',display:'grid',gap:0}}>{items.map((item,index) => {const appear=spring({frame:frame-VISUAL_ENTER_DELAY-index*11,fps,config:{damping:23,stiffness:100}}); const backgrounds=[COLORS.peach,COLORS.sky,COLORS.mint]; return <div key={item} style={{display:'grid',gridTemplateColumns:'126px 1fr',gap:22,alignItems:'stretch'}}>
...
const ChecklistVisual = ({scene}: {scene: LegalVideoScene}) => {
  ...
  return <div style={{width:'100%',display:'grid',gap:20}}>{items.map((item,index)=>{const appear=spring({frame:frame-VISUAL_ENTER_DELAY-index*10,fps,config:{damping:24,stiffness:100}}); const check=interpolate(frame,[8+index*8,20+index*8],[0,1],clamp); return <div key={item} style={{minHeight:item.length>128?210:item.length>104?190:172,padding:'27px 30px',borderRadius:38,backgroundColor:index===0?COLORS.mint:index===1?COLORS.cream:COLORS.sky,border:`2px solid ${COLORS.line}`,boxShadow:'0 17px 46px rgba(36,88,74,.08)',display:'grid',gridTemplateColumns:'116px 1fr',alignItems:'center',gap:24,opacity:appear,translate:`0 ${interpolate(appear,[0,1],[30,0],clamp)}px`}}><div style={{width:100,height:100,borderRadius:32,backgroundColor:COLORS.card,border:`3px solid ${COLORS.green}`,display:'grid',placeItems:'center'}}><svg width="64" height="64" viewBox="0 0 64 64" fill="none" aria-hidden="true"><path d="M13 33 26 46 52 17" stroke={COLORS.green} strokeWidth="7" strokeLinecap="round" strokeLinejoin="round" pathLength="1" strokeDasharray="1" strokeDashoffset={1-check}/></svg></div><div style={{fontSize:item.length>128?26:item.length>104?28:item.length>90?30:34,lineHeight:1.25,fontWeight:850,color:COLORS.ink}}>{item}</div></div>;})}</div>;
};
